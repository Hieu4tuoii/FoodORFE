...
export default function CartPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="min-h-screen pb-40 mx-auto bg-white md:pb-32">
        <div className="flex items-center p-4 md:p-6">
          <button onClick={() => navigate(-1)} className="mr-3 md:mr-4">
            <ChevronLeft size={24} className="md:w-7 md:h-7 lg:w-8 lg:h-8" />
          </button>
          <h1 className="text-lg font-medium md:text-xl lg:text-2xl">Giỏ hàng</h1>
        </div>

        <div className="flex max-w-4xl mx-auto mb-4 border-b md:mb-6">
          <button
        ...
        {error && (
          <div className="max-w-4xl px-4 py-4 mx-auto mb-4 text-center text-red-500 rounded-lg bg-red-50">
            <p>{error}</p>
            <button
              ...
              <div className="max-w-4xl px-4 mx-auto md:px-6">
                {!cart || cart.length === 0 ? (
                  <div className="py-10 text-center md:py-16">
                    <p className="text-base text-gray-500 md:text-xl">Giỏ hàng của bạn đang trống</p>
                    <button
                      className="px-6 py-2 mt-4 text-base text-white bg-orange-500 rounded-full md:mt-6 md:px-8 md:py-3 md:text-lg"
                      onClick={() => navigate("/")}
                    >
                      Tiếp tục mua sắm
                    </button>
                  ...
                  <>
                    <div className="mb-6 space-y-4 md:space-y-6 md:mb-8">
                      {cart.map((item) => (
                        <div key={item.id} className="flex items-center p-3 bg-gray-50 md:p-5 rounded-xl">
                          <img
                            src={item.image || "/placeholder.svg"}
                            alt={item.name}
                            className="object-cover w-16 h-16 rounded-lg md:w-20 md:h-20 lg:w-24 lg:h-24"
                          />
                          <div className="flex-1 ml-3 md:ml-5 lg:ml-6">
                            <h3 className="text-base font-medium md:text-lg lg:text-xl">{item.name}</h3>
                            <p className="text-sm text-red-500 md:text-base lg:text-lg">
                              {item.price.toLocaleString()}đ
                            </p>
                          </div>
                          <div className="flex items-center">
                            <button
                              className="flex items-center justify-center w-8 h-8 text-white bg-orange-500 rounded-full md:w-9 md:h-9 lg:w-10 lg:h-10"
                              onClick={() => handleUpdateQuantity(item.id, Math.max(1, item.quantity - 1))}
                              disabled={loading}
                            >
                              <Minus size={14} className="md:w-4 md:h-4 lg:w-5 lg:h-5" />
                            </button>
                            <span className="w-8 text-sm text-center md:w-10 lg:w-12 md:text-base lg:text-lg">
                              {item.quantity}
                            </span>
                            <button
                              className="flex items-center justify-center w-8 h-8 text-white bg-orange-500 rounded-full md:w-9 md:h-9 lg:w-10 lg:h-10"
                            ...
                            <button
                              className="ml-2 text-red-400 md:ml-3 lg:ml-4"
                    ...
                    <div className="flex items-center justify-between px-2 py-4 mb-6 border-t border-b md:mb-8">
                      <span className="text-lg font-medium md:text-xl">Tổng</span>
                      <span className="text-xl font-bold md:text-2xl">{totalAmount.toLocaleString()}đ</span>
                    </div>
              ...
              <div className="max-w-4xl px-4 mx-auto md:px-6">
                {orderHistory.length === 0 ? (
                  <div className="py-10 text-center">
                    ...
                    <button className="px-4 py-2 mt-4 text-white bg-orange-500 rounded-lg" onClick={loadOrders}>
                      Tải lại
                    </button>
                  </div>
                ) : (
                  <>
                    {orderHistory.map((order) => (
                      <div key={order.id} className="p-4 mb-4 bg-gray-50 rounded-xl md:p-6 md:mb-6">
                          ...
                          {order.orderDetail.map((item) => (
                            <div key={item.id} className="flex items-center">
                              ...
                              <div className="flex-1 ml-3 md:ml-4">
                                <h3 className="text-base font-medium md:text-lg">{item.food.name}</h3>
                                <p className="text-sm text-gray-500 md:text-base">Số lượng: {item.quantity}</p>
                        ...
                        <div className="flex justify-between pt-3 mt-3 border-t border-gray-200 md:mt-4 md:pt-4">
                          <span className="text-sm font-medium md:text-base lg:text-lg">Tổng tiền</span>
                          <span className="text-lg font-bold text-red-500 md:text-xl lg:text-2xl">
